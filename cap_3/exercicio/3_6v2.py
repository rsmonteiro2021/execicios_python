...
while True:
    ...
    if convidado == 'encerrar':
        print(f'Temos que preparar um jantar para {len(lista)} pessoas! São elas: {lista.title()}')
        break
    else:
        lista.append(convidado)
        print(f'Você convidou {convidado.title()} para o jantar')
while x <= (len(lista) - 1):
    print(f'Sr(a) {lista[x].title()} você está sendo convidado(a) para jantar conosco amanhã as 20h')
    x += 1
while True:
...
while True:
    excluido = input(f'''Sua lista possui {len(lista)} convidados, são eles:
{lista}!
Quem você deseja excluir? (digite E para encerrar: ''')
    if excluido == 'e' or excluido == 'E':
        print(f'Sua nova lista de convidados é composta por: \n{sorted(lista)}')
        break
    else:
